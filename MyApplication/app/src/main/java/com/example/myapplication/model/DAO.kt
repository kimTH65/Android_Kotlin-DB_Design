...
import com.example.myapplication.model.Response.loadSumCategoryEI
import com.example.myapplication.model.Response.loadSumEI
import com.example.myapplication.model.Response.loadSumII
...
interface DAO {
    ...
    //Main Category - 데이터 삭제
    @Delete
    fun deleteSubCategory(entity: SubCategory);
    ...
    @Query("SELECT substr(datetime,1,7) AS date," +
            ...
            "WHERE datetime Like :select||'%' " +
            "GROUP BY S.main_category_id " +
            "ORDER BY S.main_category_id ASC;")
    fun loadMonthSumMainCategoryEI(select:String ) : List<loadSumCategoryEI>
    ...
    @Query("SELECT substr(datetime,1,7) AS date, " +
            ...
            "ORDER BY E.expense_item_id ASC;")
    fun loadMonthSumSubCategoryEI(select:String ) : List<loadSumCategoryEI>
